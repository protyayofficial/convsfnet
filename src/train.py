...
parser.add_argument('--task-name', default='disaster_types', type=str, help='name of the task')
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, device=None, num_epochs=100, curr_epoch=1,
    ...
    for epoch in range(curr_epoch, num_epochs + 1):
        ...
        for phase in phases:
            ...
            for idx, (inputs, labels) in tqdm(enumerate(dataloaders[phase]), total=len(dataloaders[phase]),
                                              desc="[{}/{}] {} Iteration".format(epoch, num_epochs, phase.upper())):
...
def main():
    use_gpu = "cuda:2" if torch.cuda.is_available() else "cpu"
